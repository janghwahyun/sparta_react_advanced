...
const initialPost = {
  id: 0, //우리가 자동아이디생성하게 해놨잖아(yF89rAzAipeggY3CFQEi)
  user_info: {
    user_name: '기본',
    user_profile:
...
const addPostFB = () => {}; //파이어스토어에 input저장 추가 함수
    ...
    postDB.get().then(docs => {
      ...
      docs.forEach(doc => {
        ...
        let post = Object.keys(_post).reduce(
          ...
          { id: doc.id, user_infp: {} } // user_info로 묶어줘야하는 정보는???
        );
        post_list.push(post);

        // !!! 이렇게 해도 되지만 좀 더 멋지게 코드를 짤 수 없을까? !!!
        // let _post = {
        //   id: doc.id,
        //   ...doc.data(),
        // };
...
export default handleActions(
  {
    [SET_POST]: (state, action) =>
      produce(state, draft => {
        // daft에 리스트를 갈아끼울거야???? 이걸하면 nextstate상태값이 바뀌네>?? 왜?? =>shouldComponentUpdate 현재 state와 변경되는 state를 비교하여 리렌더링을 할 것인지를 결정 nextstate=변경되는 state, this.state = 현재 state 리턴값이 true일 경우 리렌더링, false일 경우는 리렌더링x
